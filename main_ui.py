                ...
                with st.expander(f"[{res['score']}점] {res['name']} ({res['symbol']})"):
                    ...
                    if st.button("➕ 내 리스트에 추가 (일반 종목으로)", key=f"add_screen_{res['symbol']}_{idx}"):
                        if res['symbol'] not in portfolio:
                            portfolio[res['symbol']] = {"price": 0.0, "qty": 0, "target": 0.0, "name": res['name'], "note": "", "types": ["general"], "in_account": False}
                            save_portfolio(current_user, portfolio)
...
with tab3:
    ...
    for idx, res in enumerate(general_results):
        ...
        change = today_res['Close'] - prev_close
        ...
        expander_title = f"🔍 {res['name']} ({res['symbol']}) | 현재가: {today_res['Close']:,.0f}원 {trend_str} | 시가: {today_res['Open']:,.0f}원"
        ...
        with st.expander(expander_title, expanded=False):
            ...
            if not is_in_account:
                if st.button(f"💰 이 종목을 '내 계좌'에 추가하여 실전 관리 시작", key=f"add_acc_{res['symbol']}_{idx}"):
                    ...
                    portfolio[res['symbol']]['price'] = res['today']['Close']
                    portfolio[res['symbol']]['target'] = res['today']['Close'] * 1.15 # 기본 15% 수익 목표 자동설정
                    save_portfolio(current_user, portfolio)
...
with tab4:
    ...
    for idx, res in enumerate(recom_results):
        ...
        expander_title = f"🌟 {res['name']} ({res['symbol']}) | 현재가: {curr_price:,.0f}원 {trend_str} | 시가: {today_res['Open']:,.0f}원"
        ...
        with st.expander(expander_title, expanded=True):
            ...
            # 💡 자동 단가 추천 로직 (0원이면 현재가 기준 +15%로 자동 표시)
            saved_price = float(p_data.get('price', 0))
            default_price = saved_price if saved_price > 0 else curr_price
            new_price = st.number_input("추천 매수 단가 (원)", value=default_price, step=100.0, key=f"y_p_{sym}_{idx}")
            
            new_qty = st.number_input("매수 수량 (주)", value=int(p_data.get('qty', 0)), step=1, key=f"y_q_{sym}_{idx}")
            
            saved_target = float(p_data.get('target', 0))
            default_target = saved_target if saved_target > 0 else new_price * 1.15
            new_target = st.number_input("목표 단가 (원)", value=default_target, step=100.0, key=f"y_t_{sym}_{idx}")
...
with tab5:
    ...
    # 💡 내 계좌에 추가(in_account=True)된 종목만 필터링해서 보여줌
    account_results = [r for r in general_results if portfolio.get(r['symbol'], {}).get('in_account', False)]

    if not account_results:
        st.info("현재 계좌에서 관리 중인 종목이 없습니다. '심층 분석' 탭 하단에서 실전 투자 종목을 추가해 보세요.")

    for idx, res in enumerate(account_results):
        sym = res['symbol']
        ...
        expander_title = f"💼 {res['name']} ({res['symbol']}) | 현재가: {curr_price:,.0f}원 {trend_str}"
        ...
        with st.expander(expander_title, expanded=True):
            ...
            saved_price = float(p_data.get('price', 0))
            default_price = saved_price if saved_price > 0 else curr_price
            new_price = st.number_input("매수 단가 (원)", value=default_price, step=100.0, key=f"gen_p_{sym}_{idx}")
            
            new_qty = st.number_input("보유 수량 (주)", value=int(p_data.get('qty', 0)), step=1, key=f"gen_q_{sym}_{idx}")
            
            saved_target = float(p_data.get('target', 0))
            default_target = saved_target if saved_target > 0 else new_price * 1.15
            new_target = st.number_input("목표 단가 (원)", value=default_target, step=100.0, key=f"gen_t_{sym}_{idx}")
